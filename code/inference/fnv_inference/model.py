...
class MaskRCNN():
    ...
    def detect(self, images, verbose=0):
        ...
        pred_gpu_st = time.time()
        detections, mrcnn_class, mrcnn_bbox, mrcnn_mask, \
            rois, rpn_class, rpn_bbox =\
            self.keras_model.predict([molded_images, image_metas], verbose=0)
        pred_gpu_et = time.time()
        # LOGI('[TIME] Products prediction in a frame gpu time is %s seconds' % (pred_gpu_et - pred_gpu_st))
        # Process detections
        results = []
        pred_cpu_st = time.time()
        ...
        pred_cpu_et = time.time()
        # LOGI('[TIME] Products prediction in a frame cpu time is %s seconds' % (pred_cpu_et - pred_cpu_st))
        return results
